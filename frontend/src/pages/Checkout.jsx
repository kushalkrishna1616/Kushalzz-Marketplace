...
export default function Checkout() {
  ...
  const handlePaymentAndPlaceOrder = async () => {
    ...
    if (paymentMethod === "cod") {
      ...
      try {
        ...
        clearCart();
        toast.success("Boutique Order placed (COD)");
  ...
  const handleMockSuccess = async () => {
    setIsPaying(true);
    try {
      // Create backend order just to mimic real flow
      await API.post("/orders", {
        items: cart,
        address: selectedAddress,
        total,
        paymentStatus: "paid",
        transactionId: "ELITE_TXN_" + Math.floor(Math.random() * 100000000)
      });
      clearCart();
      toast.success("Elite Transaction Approved! Order Finalized.");
      setShowMockModal(false);
      navigate("/order-success");
    } catch (err) {
      toast.error("Failed to confirm order with backend.");
    } finally {
      setIsPaying(false);
    }
  };
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-28 bg-white rounded-2xl md:rounded-3xl p-8 md:p-10 shadow-[0_30px_70px_rgba(0,0,0,0.05)] border border-gray-50 overflow-hidden">
              <div className="absolute top-0 right-0 p-3 opacity-5">
                <h2 className="text-6xl font-bold uppercase rotate-90" style={{ fontFamily: "'Jost', sans-serif" }}>ELITE</h2>
              </div>
              ...
              <div className="space-y-4 py-8 border-t border-gray-50">
                ...
                <div className="flex justify-between items-end pt-4 border-t border-gray-50">
                   ...
                   <div className="text-right">
                      <p className="text-[9px] font-bold text-green-500 uppercase tracking-widest mb-1">+ K-Credits</p>
              ...
              >
                ...
                {isPaying ? "Processing Payment..." : isPlacingOrder ? "Restocking Inventory..." : `Secure Checkout ₹${total}`}
              </button>

  ...
  const handleSendMessage = async (e) => {
    ...
    if (mode === "human") {
      ...
      try {
        ...
        const serverMsg = err.response?.data?.reply || err.message;
        const errorReply = {
          sender: "AI",
          receiver: activeId,
          senderName: "Chillzz AI",
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-sm font-bold tracking-tight uppercase" style={{ fontFamily: "'Jost', sans-serif" }}>
                  {mode === "ai" ? "Chillzz AI" : "Chillzz Concierge Support"}
                </h3>
                <p className="text-[10px] text-gray-400">
                  {mode === "ai" ? "Powered by Kushalzz Elite Intelligence" : "Average response: 5 mins"}
                </p>
              </div>
            </div>
          </div>
          ...
          {chat.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-6">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                ...
                {mode === "ai" 
                  ? "I'm Chillzz AI. Ready to curate your next masterpiece from our elite collections?"
                  : "Need assistance with a premium order? Our Chillzz concierge is at your service."}
              </p>
            </div>
          ) : (
            chat.map((msg, idx) => (
